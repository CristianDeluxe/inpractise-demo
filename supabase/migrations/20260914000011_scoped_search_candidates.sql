...
create function public.search_candidates_scoped(query_text text,query_embedding extensions.vector(1536) default null,company_filter text default null,candidate_limit integer default 30)
...
revoke all on function public.search_candidates_scoped(text,extensions.vector,text,integer) from public,anon,authenticated;
grant execute on function public.search_candidates_scoped(text,extensions.vector,text,integer) to authenticated;
