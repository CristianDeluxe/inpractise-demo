...
export function LibraryPanel({ library, company }: LibraryPanelProps) {
  const documents = library.items.filter(
    (document) => !company || document.company === company,
  )
  return (
    <section id="library">
      <div className="mb-5 flex flex-wrap items-end justify-between gap-3">
        ...
        <p className="text-sm text-muted-foreground">
          {company
            ? `${String(documents.length)} of ${String(library.items.length)} authorized documents`
            : `${String(library.items.length)} authorized documents`}
        </p>
      </div>
