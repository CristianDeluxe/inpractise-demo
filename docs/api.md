...
| `POST /api/v1/answers`                                                           | JSON `query`, optional `company`; returns `status`, `claims`, `missingEvidence`, `citations`, `mode`, `candidateCount`. **Consumes the caller's backend request allowance.** |
