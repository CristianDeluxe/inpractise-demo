...
- `search_research(query, company?, limit?)`: ranked passages with their
  immutable citations.
- `fetch_passage(documentId, revisionId, passageId)`: one whole passage plus its
  neighbours' IDs.

It signs in as an ordinary member with a password and calls the same
`POST /functions/v1/research` endpoint the browser calls. It holds no service
key, and the tools take no org, user or role argument: what the member may read
is decided by the same row level security as in the browser. The Node server
writes only protocol messages to stdout and diagnostics to stderr. The
`pnpm mcp` wrapper also prints a package-script banner; agent clients launch
Node directly, as shown in the install guide.
...
advertised for the browser demo. It reads `RESEARCH_URL`,
`RESEARCH_PUBLISHABLE_KEY` and `DEMO_MCP_PASSWORD` directly, and uses that fixed
email and password unless a generic client supplies `RESEARCH_EMAIL` or
`RESEARCH_PASSWORD`. Client JSON may map the direct names to resolved names.
Credentials arrive only through the environment, never as a tool argument or a
command-line flag. The optional `RESEARCH_HANDSHAKE_LOG` records each negotiated
session.
...
`1e899018-7c9d-4568-ae7a-59d79df71039`, 2026-09-13, five turns, 20.0s, asking
for the Meridian January 2026 example and then for February. Its own answer, in
part:
...
The [handshake JSONL](mcp-handshake.jsonl) is the unchanged artifact of the
2026-09-13 session, captured before the 2026-09-14 project rename under the
pre-rename registration described in ADR 0001. Its two records contain
timestamps and negotiated protocol versions only. It is kept as captured; a
post-rename session gets its own log.
